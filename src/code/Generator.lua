...
local function generateModuleScript(name, destination, headers, rows)
	...
	if not scc then
		Logger.error("Failed to parent ModuleScript to " .. destination.Name .. ": " .. tostring(scc))
		return nil
	end
...
local function generateLuaTable(name, destination, headers, rows)
	...
	if not scc then
		Logger.error("Failed to parent Script to " .. destination.Name .. ": " .. tostring(scc))
		return nil
	end
...
function Generator.generate(options: any): (Instance?, string?)
	...
	if outputType == "ModuleScript" then
		instance = generateModuleScript(name, dest, headers, rows)
	elseif outputType == "Folder + Values" then
		instance = generateFolderValues(name, dest, headers, rows)
	elseif outputType == "Lua Table" then
		instance = generateLuaTable(name, dest, headers, rows)
	else
